...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE
    # squaring our x and y direction gradients and then apply a blur
    dx, dy = dx ** 2, dy ** 2
    dxy = dx * dy
    mx = convolve(dx, window, mode='constant') 
    my = convolve(dy, window, mode='constant')
    mxy = convolve(dxy, window, mode='constant')
...
def simple_descriptor(patch):
    ...
    new_feature = patch.flatten()
    new_feature = new_feature - np.mean(new_feature)
    
    feature = new_feature / np.std(new_feature)
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    N = desc1.shape[0]
    descs = cdist(desc1, desc2)

    ### YOUR CODE HERE
    
    matches = np.concatenate([np.arange(N).reshape(-1, 1), np.argmin(descs, axis=1).reshape(-1, 1)], axis=1)
    d = np.sort(descs, axis=1)
    
    # appending all the "good" matches (over the threshold)
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    # storing the current maximum inlier values
        max_inliers = np.zeros(N, dtype=np.int32)
        temp_n = 0 
        # random selecting index
